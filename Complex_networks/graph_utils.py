        ...
        for sentence in text:
            for word, vector in sentence["sentence_text"].items():
                if word not in vertices.keys():
                    try:
                        vertices[word] = Node(
                            word=word, vector=np.array(vector[:k]), neighbors=set()
                        )
                    except:
                        pass
    file_.close()
        ...
        for sentence in text:
            for word, vector in sentence["sentence_text"].items():
                ...
                if word in vertices.keys():
                    ...
                    words_text_neighbors[word].sentence_neighbors.update(
                        word_sentence_neighbors
                    )
                else:
                    try:
                        words_text_neighbors[word] = WordNeighbors(
                            word=word,
...
class Graph:
    ...
    def update_graph_by_document_neighbors(self, text_neighbors: dict):
        ...
        for word in tqdm(self.vertices.keys()):
            self.vertices[word].neighbors.intersection_update(
                text_neighbors[word].document_neighbors
            )
    ...
    def update_graph_by_sentence_neighbors(self, text_neighbors: dict):
        ...
        for word in tqdm(self.vertices.keys()):
            self.vertices[word].neighbors.intersection_update(
                text_neighbors[word].sentence_neighbors
            )
    ...
    def get_eccentricity(self) -> dict:
        ...
        for word in tqdm(graph.nodes, file=file_):
            ...
            for word in short_paths.keys():
                short_paths_full[word] = tuple(
                    [short_paths[word], short_paths_length[word]]
                )
    ...
    def get_eccentricity_for_words(self, words: set) -> dict:
        ...
        for word in tqdm(words, file=file_):
            ...
            for other_word in short_paths.keys():
                short_paths_full[other_word] = tuple(
                    [short_paths[other_word], short_paths_length[other_word]]
                )
            eccentricity[word] = short_paths_full
        file_.close()
        return eccentricity
    ...
    def dijkstra(self, word: str) -> list:
        ...
        dijkstra_nodes = [
            DijkstraNodeDecorator(other_word, index, self.vertices[other_word])
            for index, other_word in enumerate(words_list)
        ]
        dijkstra_nodes[word_node_index].provisional_distance = 0
        dijkstra_nodes[word_node_index].hops.append(self.vertices[word])
        is_less_than = (
            lambda dijkstra_node_1, dijkstra_node_2: dijkstra_node_1.provisional_distance
            < dijkstra_node_2.provisional_distance
        )
        ...
        while heap.get_word_nodes_size() > 0:
            ...
            for (neighbor_index, edge_weight) in connections:
                ...
                if heap_location is not None:
                    ...
                    if (
                        tot_dist
                        < heap.dijkstra_nodes[heap_location].provisional_distance
                    ):
                        hops_cpy = list(hops)
...
class MinHeap(WordsBinaryTree):
    ...
    def min_heapify_subtree(self, node_index: int):
        ...
        if left_index < size and self.is_less_than(
            self.dijkstra_nodes[left_index], self.dijkstra_nodes[minimum_index]
        ):
            minimum_index = left_index
        if right_index < size and self.is_less_than(
            self.dijkstra_nodes[right_index], self.dijkstra_nodes[minimum_index]
        ):
        ...
        if minimum_index != node_index:
            ...
            if self.get_index is not None:
                self.order_mapping[
                    self.dijkstra_nodes[minimum_index].index
                ] = minimum_index
    ...
    def decrease_key(self, index, data):
        ...
        while index != 0 and not self.is_less_than(
            ...
            if self.get_index is not None:
                self.order_mapping[
                    self.dijkstra_nodes[parent_index].index
                ] = parent_index
...
class Parallel(Graph):
    ...
    def is_neighbor_in_edge_sphere(self, edge: tuple, neighbor: str) -> bool:
        ...
        """
        sphere_center = (
            self.vertices[edge[0]].vector + self.vertices[edge[1]].vector
        ) / 2
        ...
        for word in tqdm(self.parallel, file=file_):
            ...
            for neighbor in neighbors:
                for other_word in self.get_other_words(word, neighbor):
                    ...
                    neighbor_to_other_word_distance = self.euclid_distance(
                        neighbor, other_word
                    )
                    if (
                        max(
                            word_to_other_word_distance, neighbor_to_other_word_distance
                        )
                        <= word_to_neighbor_distance
                    ):
...
class GG(Graph):
    ...
    def create_parallel_gabriel_graph(self, triangles: list, num_cpus: int = 1):
        ...
        results = ray.get(
            [
                actor.parallel_gabriel_graph.remote(index)
...
class RNG(GG):
    ...
    def create_rn_graph_for_words(
        self, words: set, already_gabriel: bool = True
    ) -> dict:
        """
    ...
    def create_parallel_rn_graph(
        self, num_cpus: int = 1, use_input_as_gabriel: bool = True
    ):
        """
        Create parallel RNG
        ...
        results = ray.get(
            [
                actor.parallel_rn_graph.remote(index)
                for index, actor in enumerate(streaming_actors)
            ]
        )
        ray.shutdown()
        ...
        for parallel_vertices in tqdm(results, file=file_):
            for word in self.get_words():
                self.vertices[word].neighbors.intersection_update(
                    parallel_vertices[word].neighbors
                )
        file_.close()
...
class EBall(Graph):
    ...
    def create_parallel_eball_graph(self, epsilon: float, num_cpus: int):
        ...
        results = pool.map(
            lambda actor, index: actor.parallel_eball_graph.remote(epsilon, index),
            indexes,
        )
        ray.shutdown()
